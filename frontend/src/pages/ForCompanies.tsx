  ...
  return (
    <div className="min-h-screen bg-[#F4F4F7] text-[#0E2A38] antialiased" style={{ fontFamily: "Manrope, ui-sans-serif, system-ui" }}>
      ...
      <section className="relative overflow-hidden bg-[#F4F4F7]">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-10 pt-20 pb-16 grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <p className="text-[#0E2A38]/65 text-base leading-relaxed mb-8 max-w-md">
            ...
            <div className="flex flex-wrap gap-3">
              <Link to="/contact" className="inline-flex items-center gap-1.5 rounded-full bg-[#0B74B0] hover:bg-[#096396] text-white px-5 py-2.5 text-sm font-medium transition shadow-sm">
                Book a Free Consultation <ArrowUpRight size={16} />
              </Link>
              <Link to="/services" className="inline-flex items-center rounded-full border border-black/15 text-[#0E2A38] hover:bg-black/5 px-5 py-2.5 text-sm font-medium transition">
                View Services
              </Link>
            </div>
          </div>

          <div className="rounded-2xl bg-white shadow-[0_20px_60px_-15px_rgba(14,42,56,0.25)] p-6 border border-black/5">
            <div className="text-[#0B74B0] text-xs font-semibold tracking-[0.2em] uppercase mb-5">The Vantage Point Advantage</div>
            <div className="space-y-3 mb-6">
              {["Pre-vetted candidates in 48–72 hours", "Only top 3% of applicants reach you", "90-day replacement guarantee", "Dedicated account manager", "Full IT services at a flat monthly rate"].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-xl border border-black/5 bg-[#F4F4F7] p-3">
                  <CheckCircle size={15} className="text-[#0B74B0] flex-shrink-0" />
                  <span className="text-sm text-[#0E2A38] font-medium">{item}</span>
                </div>
              ))}
            </div>
            <Link to="/contact" className="block w-full h-11 leading-[44px] text-center rounded-xl bg-[#0E2A38] text-white text-sm font-medium hover:bg-[#0a1f29] transition">
